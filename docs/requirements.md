...
- 文本回复与实时通话复用同一份按角色回答语言生成的全局系统提示词；舞台表现为可选内容，中文使用全角括号，英文使用半角括号。
...
- 消息语音和翻译缓存按用户保留最近 20 项；角色语言或文本变化会导致语音缓存失效。
...
- 实时配置接口返回 WebSocket 地址、资源 ID、角色 speaker ID、角色语言、角色指令和 speaking style，不返回供应商访问令牌。
