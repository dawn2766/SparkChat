...
DEEPSEEK_RECENT_CONTEXT_MAX_TOKENS = int(os.getenv("DEEPSEEK_RECENT_CONTEXT_MAX_TOKENS", "480000"))
DEEPSEEK_MEMORY_UPDATE_INTERVAL_TOKENS = int(os.getenv("DEEPSEEK_MEMORY_UPDATE_INTERVAL_TOKENS", "440000"))
VOICE_RECENT_CONTEXT_MAX_TOKENS = int(os.getenv("VOICE_RECENT_CONTEXT_MAX_TOKENS", "9000"))
VOICE_MEMORY_UPDATE_INTERVAL_TOKENS = int(os.getenv("VOICE_MEMORY_UPDATE_INTERVAL_TOKENS", "8000"))
...
if not 0 < DEEPSEEK_MEMORY_UPDATE_INTERVAL_TOKENS < DEEPSEEK_RECENT_CONTEXT_MAX_TOKENS:
    raise ValueError(
        "DEEPSEEK_MEMORY_UPDATE_INTERVAL_TOKENS must be smaller than DEEPSEEK_RECENT_CONTEXT_MAX_TOKENS"
    )
if not 0 < VOICE_MEMORY_UPDATE_INTERVAL_TOKENS < VOICE_RECENT_CONTEXT_MAX_TOKENS:
    raise RuntimeError(
        "Voice token limits must satisfy 0 < memory interval < recent context maximum"
    )


def fallback_token_count(content):
    text = str(content or "")
...
def message_token_count(message):
    ...
    return max(1, int(token_count) if token_count is not None else fallback_token_count(message["content"]))


def select_recent_messages(messages, covered_through_message_id, max_tokens=RECENT_CONTEXT_MAX_TOKENS):
    ...
    for index in range(len(messages) - 1, -1, -1):
        ...
        if not is_required and selected_tokens + token_count > max_tokens:
            break
...
def select_memory_batch(messages, covered_through_message_id, interval_tokens=MEMORY_UPDATE_INTERVAL_TOKENS):
    stable_messages = stable_messages_for_memory(messages, covered_through_message_id)
    if sum(message_token_count(message) for message in stable_messages) < interval_tokens:
        return []
    batch = []
    batch_tokens = 0
    for message in stable_messages:
        token_count = message_token_count(message)
        if batch and batch_tokens + token_count > interval_tokens:
            break
...
def should_update_memory(messages, covered_through_message_id, interval_tokens=MEMORY_UPDATE_INTERVAL_TOKENS):
    return bool(select_memory_batch(messages, covered_through_message_id, interval_tokens))
