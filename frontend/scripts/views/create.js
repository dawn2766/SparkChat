...
export function renderCreate({ bindShell, onCreated }) {
  ...
  const content = `<form id="create-form">
    ...
    <section class="section"><div class="section-header"><div class="section-title"><h2>角色音色</h2></div></div><div class="voice-picker"><div class="field"><label for="voice-select">音色</label><select class="select-input" id="voice-select" name="voiceId">${voiceOptions(selected?.id)}</select><input type="hidden" name="voiceName" value="${esc(selected?.name || "")}"></div></div></section>
