...
export function renderAuth(error = "", onAuthenticated) {
  ...
  app.innerHTML = `<main class="auth-view"><section class="auth-content"><header class="auth-brand"><img src="assets/images/sparkchat-logo.png" alt=""><span>SPARKCHAT</span></header><div class="auth-heading"><h1>${isLogin ? "欢迎回来" : "创建你的账号"}</h1><p>${isLogin ? "登录以继续你的对话" : "一个账号，保存你的角色与对话"}</p></div><form class="auth-form" id="auth-form" autocomplete="off" data-form-type="other" novalidate><div class="field"><label for="username">账号</label><input class="text-input" id="username" name="sparkHandle" required minlength="${isLogin ? 1 : AUTH_RULES.username.min}" maxlength="${AUTH_RULES.username.max}" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="${isLogin ? "输入你的账号" : "3–24 个字符"}" aria-describedby="username-error"><p class="field-error" id="username-error"></p></div><div class="field"><label for="password">密码</label><div class="password-control"><input class="text-input" id="password" name="sparkSecret" type="password" required minlength="${isLogin ? 1 : AUTH_RULES.password.min}" maxlength="${AUTH_RULES.password.max}" autocomplete="new-password" autocapitalize="none" spellcheck="false" placeholder="${isLogin ? "输入你的密码" : "至少 4 个字符"}" aria-describedby="password-error"><button class="icon-button password-toggle" id="password-toggle" type="button" aria-label="显示密码" aria-pressed="false"><i data-lucide="eye" aria-hidden="true"></i></button></div><p class="field-error" id="password-error"></p></div><div class="auth-message${error ? " is-visible" : ""}" id="auth-message" role="alert" aria-live="polite">${esc(error)}</div><button class="primary-button full-width auth-submit" id="auth-submit">${submitLabel}</button><div class="auth-divider"><span>或</span></div><button class="auth-switch" id="auth-switch" type="button">${isLogin ? "还没有账号？" : "已有账号？"} <strong>${isLogin ? "创建账号" : "直接登录"}</strong></button></form></section><footer class="auth-footer"><a href="http://beian.miit.gov.cn/" target="_blank" rel="noopener noreferrer">闽ICP备2024055854号</a></footer></main>`;
  ...
  formElement.onsubmit = async (event) => {
    ...
    const username = String(formData.get("sparkHandle") || "").trim();
    const password = String(formData.get("sparkSecret") || "");
